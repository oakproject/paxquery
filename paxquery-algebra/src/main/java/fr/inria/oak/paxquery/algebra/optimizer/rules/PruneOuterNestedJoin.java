...
public final class PruneOuterNestedJoin implements Logical2Logical {

  public static final PruneOuterNestedJoin INSTANCE = new PruneOuterNestedJoin();

  private PruneOuterNestedJoin() {
  }

  @Override
  public BaseLogicalOperator transform(BaseLogicalOperator operator) {
    return pruneOuterNestedJoin(operator);
  }

  private static BaseLogicalOperator pruneOuterNestedJoin(BaseLogicalOperator operator) {
    if (operator instanceof Projection
      ...
      final LeftOuterNestedJoin lonj = (LeftOuterNestedJoin) operator.getChildren().get(0);
