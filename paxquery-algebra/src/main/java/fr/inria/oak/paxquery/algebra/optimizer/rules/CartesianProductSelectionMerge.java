...
public final class CartesianProductSelectionMerge implements Logical2Logical {

  public static final CartesianProductSelectionMerge INSTANCE =
          new CartesianProductSelectionMerge();


  private CartesianProductSelectionMerge() {}

  @Override
  public BaseLogicalOperator transform(BaseLogicalOperator root) {
    traversePlanRecursive(root);
    return root;
  }

  /*
   * private void traversePlanRecursive(BaseLogicalOperator root) { ArrayList<BaseLogicalOperator>
   * children = root.getChildren(); if(children != null && children.size() > 0) { if(root instanceof
   * Selection && children.get(0) instanceof CartesianProduct) { //do the actual operator
   * replacement BaseLogicalOperator result = replaceOperators((Selection)root,
   * (CartesianProduct)children.get(0)); if(result != null) { root = result; children =
   * root.getChildren(); } } //keep traversing the plan recursively. The base case is when children
   * == null || children.size() == 0 for(BaseLogicalOperator child : children)
  ...
  private void traversePlanRecursive(BaseLogicalOperator root) {
    ...
    if (children != null && children.size() > 0) {
      if (root instanceof Selection && children.get(0) instanceof CartesianProduct) {
        ...
        BaseLogicalOperator result = replaceOperators((Selection) root,
                (CartesianProduct) children.get(0));
        if (result != null) {
          root = result;
        ...
        // keep traversing the plan recursively. The base case is when children == null ||
        // children.size() == 0
  ...
  private void replaceOperators(Join join, CartesianProduct cp) {
    ...
    for (int conjIndex = conjPreds.size() - 1; conjIndex >= 0; conjIndex--) {
      ...
      for (int simpleIndex = simplePreds.size() - 1; simpleIndex >= 0; simpleIndex--) {
        ...
        if (isVarInTreePatternList(cp.getNavigationTreePatterns(), simplePred.getVariable1())
                && isVarInTreePatternList(cp.getNavigationTreePatterns(), simplePred.getVariable2())) {
  ...
  /**
   * Replaces the sel-cp construction by a join, and returns the join if the replacement was
   * actually done. Otherwise, it returns null.
   * 
   * @param sel
   * @param cp
   * @return
